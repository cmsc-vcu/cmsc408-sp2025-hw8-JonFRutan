...
run_sql_and_return_html(cnx,"""
select Region from wdi_country group by Region order by Region
""")
...
run_sql_and_return_html(cnx,"""
select Region, count(*) as `Count` from wdi_country group by Region order by Count desc
""")
...
run_sql_and_return_html(cnx,"""
select `Long Name`, Region FROM wdi_country WHERE `Short Name` = "Qatar"
""")
...
from wdi_country group by `Income Group`
...
run_sql_and_return_html(cnx,"""
select `Short Name`, `Income Group` from wdi_country where `Income Group` is null and Region is not null
""")
...
run_sql_and_return_html(cnx,"""
select `Long Name`, `Income Group` from sp25_rutanj_user.wdi_country where `Short Name` = "Venezuela"
""")
...
count(*) as Total
from wdi_country group by Region order by Region
""")
...
run_sql_and_return_html(cnx,"""
with pairings as (select r.Region, i.`Income Group` from (select distinct Region from wdi_country where Region is not null) r cross join (select distinct `Income Group` from wdi_country where `Income Group` is not null) i),
existing_pairs as (select Region, `Income Group` from wdi_country group by Region, `Income Group`)
select pairings.Region, pairings.`Income Group` from pairings left join existing_pairs
...
run_sql_and_return_html(cnx,"""
with counts as (
select Region, `Income Group`, count(*) as CountryCount from wdi_country
where Region is not null and `Income Group` is not null
group by Region, `Income Group`),
region_totals as (
select Region, count(*) as region_total from wdi_country
where Region is not null and `Income Group` is not null
group by Region),
income_totals as (
select `Income Group`, count(*) as income_total from wdi_country
where Region is not null and `Income Group` is not null
group by `Income Group`),
totals as (
select count(*) as total from wdi_country
where Region is not null and `Income Group` is not null)
select c.region, c.`Income Group`, r.region_total, i.income_total, t.total, round((c.CountryCount/t.total) * 100, 2) as PercentageTotal
from counts c
...
with total as (
select count(*) as total
from wdi_country where Region is not null and `Income Group` is not null)
select `Income Group`, count(*) as count, (select total from total) as total, round(count(*)*100.0/(select total from total), 2) as percantage from wdi_country
where Region is not null and `Income Group` is not null
group by `Income Group` order by percantage desc
""")
