...
run_sql_and_return_html(cnx,"""
select distinct Region from wdi_country group by Region order by Region
""")
...
count(*) as Total
from sp25_rutanj_user.wdi_country group by Region order by Region
""")
...
select `Income Group`, count(*) as count, (select total from total) as total, round(count(*)*100.0/(select total from total), 2) as percantage from sp25_rutanj_user.wdi_country
where Region is not null and `Income Group` is not null
